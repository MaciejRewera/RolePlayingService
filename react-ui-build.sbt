...
lazy val `react-ui-test` = TaskKey[Unit]("Run UI tests when testing application.")

`react-ui-test` := {
  implicit val userInterfaceRoot = baseDirectory.value / "react-ui"
  if (executeUiTests != Success) throw new Exception("UI tests failed!")
}

lazy val `react-ui-prod-build` = TaskKey[Unit]("Run UI build when packaging the application.")
...
dist := (dist dependsOn `react-ui-prod-build`).value

// Execute frontend prod build task prior to play stage execution.
stage := (stage dependsOn `react-ui-prod-build`).value

// Execute frontend test task prior to play test execution.
test := ((test in Test) dependsOn `react-ui-test`).value